...
//        .toLocalTime();
    ...
    public static List<UserMealWithExceed>  getFilteredMealsWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        Map<LocalDate, ArrayList<UserMeal>> mealMap = new HashMap<>();
        List<UserMealWithExceed> userMealWithExceeds = new ArrayList<>();
        for (UserMeal userMeal : mealList) {
            LocalDateTime localDateTime = userMeal.getDateTime();
            LocalDate localDate = localDateTime.toLocalDate();
            if (!mealMap.containsKey(localDate)) mealMap.put(localDate, new ArrayList<>());
            mealMap.get(localDate).add(userMeal);
        }
        for (ArrayList<UserMeal> userMeals : mealMap.values()) {
